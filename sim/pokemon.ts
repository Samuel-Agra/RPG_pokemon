...
import { RPGManager, type RPGPokemonState } from "./rpg-showdown";
...
export class Pokemon {
	...
	constructor(set: string | AnyObject, side: Side) {
		...
		// Cria uma cópia independente do estado persistente
		this.rpg = structuredClone(this.set.rpg ?? {});
		if (this.set.rpg !== undefined) {
			this.rpg.friendship = this.battle.clampIntRange(this.rpg.friendship ?? 50, 0, 255);
			this.set.happiness = this.rpg.friendship;
		}
		if (this.rpg.level !== undefined) {
			this.set.level = this.battle.clampIntRange(this.rpg.level, 1, 100);
		}
		if (this.rpg.evs) {
			this.set.evs = { ...this.set.evs, ...this.rpg.evs };
		}
		RPGManager.onPokemonSetInitialized(this);
	...
	*/
	...
	isAdjacent(pokemon2: Pokemon) {
		...
		const position = this.battle.rpg?.layout === 'raid' ? this.side.active.indexOf(this) : this.position;
		const position2 = this.battle.rpg?.layout === 'raid' ? pokemon2.side.active.indexOf(pokemon2) : pokemon2.position;
		const activePosition = position < 0 ? this.position : position;
		const activePosition2 = position2 < 0 ? pokemon2.position : position2;
		if (this.side === pokemon2.side) return Math.abs(activePosition - activePosition2) === 1;
		return Math.abs(activePosition + activePosition2 + 1 - this.side.active.length) <= 1;
	}
	...
	getLocOf(target: Pokemon) {
		...
		const raidPosition = this.battle.rpg?.layout === 'raid' ? target.side.active.indexOf(target) : -1;
		const position = (raidPosition < 0 ? target.position : raidPosition) + positionOffset + 1;
	...
	faint(source: Pokemon | null = null, effect: Effect | null = null) {
		...
		RPGManager.onPokemonFainted(this, source);
		return d;
	}
