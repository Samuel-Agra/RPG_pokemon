...
import { RPGManager } from "./rpg-showdown";

/** A single action that can be chosen. Choices will have one Action for each pokemon. */
export interface ChosenAction {
	choice: 'move' | 'switch' | 'instaswitch' | 'revivalblessing' | 'team' | 'shift' | 'pass' | 'rpgItem';// action type
	...
	rpgActionId?: string; // opaque RPG runtime action identifier
}
...
export class Side {
	...
	constructor(name: string, battle: Battle, sideNum: number, team: PokemonSet[]) {
		...
		this.pokemonLeft = this.pokemon.filter(pokemon => !pokemon.fainted).length;
	...
	addPokemon(set: PokemonSet) {
		...
		// Inicializa o estado persistente do RPG
		RPGManager.onPokemonCreated(newPokemon);
		newPokemon.position = this.pokemon.length;
		this.pokemon.push(newPokemon);
		if (!newPokemon.fainted) this.pokemonLeft++;
		return newPokemon;
	}
	...
	choose(input: string) {
		...
		for (const choiceString of choiceStrings) {
			...
			case 'team':
				...
				break;
			case 'rpgitem':
				if (!this.chooseRPGItem(data)) return false;
				break;
			case 'pass':
			case 'skip':
	...
	chooseRPGItem(actionId: string): boolean | Side {
		const index = this.getChoiceIndex();
		const pokemon = this.active[index];
		if (!this.battle.rpg || this.requestState !== 'move') {
			return this.emitChoiceError(`Can't use an RPG Bag item outside an RPG move request`);
		}
		if (!pokemon || pokemon.fainted || pokemon.volatiles['commanding']) {
			return this.emitChoiceError(`Can't use an RPG Bag item: this Pokemon cannot act`);
		}
		if (!actionId || actionId.length > 300 || actionId.includes(',')) {
			return this.emitChoiceError(`Can't use an RPG Bag item: invalid action identifier`);
		}
		this.choice.actions.push({
			choice: 'rpgItem', pokemon, rpgActionId: actionId, priority: 6,
		});
		return true;
	}
		...
		if (!isPass) {
			...
			case 'move':
				...
				while (
					...
					(!this.active[index] || this.active[index].fainted || this.active[index].volatiles['commanding'])
				) {
					this.choosePass();
					index++;
				}
	...
	choosePass(): boolean | Side {
		...
		const pokemon: Pokemon = this.active[index];
		if (!pokemon && (this.requestState === 'move' || this.requestState === 'switch')) {
			this.choice.actions.push({ choice: 'pass' });
			return true;
		}
