...
export class BattleActions {
	...
	switchIn(pokemon: Pokemon, pos: number, sourceEffect: Effect | null = null, isDrag?: boolean) {
		...
		if (oldActive) {
			RPGManager.onSwitchOut(oldActive);
	...
	runSwitch(pokemon: Pokemon) {
		...
		// O estado persistente precisa entrar antes de hazards e outros eventos nativos de SwitchIn.
		for (const poke of switchersIn) {
			if (poke.hp) RPGManager.onSwitchIn(poke);
		}
	...
	runMegaEvo(pokemon: Pokemon) {
		...
		RPGManager.onMegaEvolution(pokemon);
