...
describe('RPG Egg presentation outside incubation', () => {
	...
	it('animates a normal Egg and opens the Portable Incubator before hatching', () => {
		...
		assert.match(nurseryCss, /rpg-hatch-incubator-part\.lid\{clip-path:inset\(0 0 68% 0\)\}/);
		assert.match(nurseryCss, /rpg-hatch-incubator-part\.base\{clip-path:inset\(32% 0 0 0\)\}/);
		assert.doesNotMatch(nurseryCss, /incubator-open[^}]*rpg-hatch-egg-host\{[^}]*scale:/);
