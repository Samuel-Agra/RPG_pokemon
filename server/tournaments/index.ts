...
import {
	RPGTournamentSystem, type RPGTournamentAutomaticResult,
	type RPGTournamentNPCClass, type RPGTournamentParticipant,
} from '../../sim/rpg-showdown/systems/tournament';
...
export class TournamentPlayer extends Rooms.RoomGamePlayer<Tournament> {
	...
	games: number;
	score: number;
	rpgParticipant: RPGTournamentParticipant;
	constructor(user: User | string | null, game: Tournament, num: number) {
		...
		this.games = 0;
		this.score = 0;
		this.rpgParticipant = RPGTournamentSystem.normalizeParticipant({
			id: typeof user === 'string' ? user : user?.id || 'player-' + String(num),
			name: typeof user === 'string' ? user : user?.name || 'Player ' + String(num),
			type: 'player',
		});
	}
}
...
export class Tournament extends Rooms.RoomGame<TournamentPlayer> {
	...
	readonly rpgAutomaticResults: RPGTournamentAutomaticResult[];
	private rpgAutomaticMatchSequence: number;
	...
	constructor(
		...
		this.rpgAutomaticResults = [];
		this.rpgAutomaticMatchSequence = 0;
	...
	addUser(user: User, output: Chat.CommandContext) {
		...
		if (this.playerCap && this.players.length >= this.playerCap) {
		...
		this.isBracketInvalidated = true;
		this.update();
		if (this.players.length === this.playerCap) {
	...
	addRPGNPC(definition: { id: string, name: string, npcClass?: RPGTournamentNPCClass }) {
		if (this.isTournamentStarted) throw new Error('RPG Tournament bracket is already frozen');
		if (this.playerCap && this.players.length >= this.playerCap) throw new Error('RPG Tournament is full');
		const participant = RPGTournamentSystem.normalizeParticipant({
			id: definition.id,
			name: definition.name,
			type: 'npc',
			npcClass: definition.npcClass || 'generic',
		});
		if (this.players.some(player => player.rpgParticipant.id === participant.id)) {
			throw new Error('RPG Tournament participant is already registered: ' + participant.id);
		}
		const player = this.addPlayer(participant.name);
		if (!player) throw new Error('Failed to add RPG Tournament NPC');
		player.rpgParticipant = participant;
		this.room.add(`|tournament|join|${participant.name}`);
		this.isBracketInvalidated = true;
		this.update();
		return player;
	}

	getRPGParticipants(): RPGTournamentParticipant[] {
		return this.players.map(player => structuredClone(player.rpgParticipant));
	}

	getRPGPendingMatches() {
		if (!this.isTournamentStarted) return [];
		const matches = this.generator.getAvailableMatches();
		if (typeof matches === 'string') throw new Error('Unexpected RPG Tournament bracket state: ' + matches);
		return matches.map(([participant1, participant2]) => ({
			participant1: structuredClone(participant1.rpgParticipant),
			participant2: structuredClone(participant2.rpgParticipant),
			automatic: RPGTournamentSystem.isAutomaticMatch(
				participant1.rpgParticipant,
				participant2.rpgParticipant
			),
			requiresBattle: !RPGTournamentSystem.isAutomaticMatch(
				participant1.rpgParticipant,
				participant2.rpgParticipant
			),
		}));
	...
	resolveRPGNPCMatches(random: () => number = Math.random): RPGTournamentAutomaticResult[] {
		if (!this.isTournamentStarted) return [];
		const resolved: RPGTournamentAutomaticResult[] = [];
		let safety = 0;
		while (!this.generator.isTournamentEnded()) {
			if (++safety > 10000) throw new Error('RPG Tournament automatic match safety limit exceeded');
			const matches = this.generator.getAvailableMatches();
			if (typeof matches === 'string') throw new Error('Unexpected RPG Tournament bracket state: ' + matches);
			const match = matches.find(([p1, p2]) => (
				RPGTournamentSystem.isAutomaticMatch(p1.rpgParticipant, p2.rpgParticipant)
			));
			if (!match) break;

			const [p1, p2] = match;
			const automaticResult = RPGTournamentSystem.resolveNPCMatch(
				this.roomid + ':npc:' + String(++this.rpgAutomaticMatchSequence),
				p1.rpgParticipant,
				p2.rpgParticipant,
				random
			);
			const p1Won = automaticResult.winnerId === p1.rpgParticipant.id;
			const result = p1Won ? 'win' : 'loss';
			const score = p1Won ? [1, 0] : [0, 1];
			...
			if (error) {
				throw new Error('Unexpected ' + error + ' while resolving RPG Tournament NPC match');
			}
			this.rpgAutomaticResults.push(automaticResult);
			resolved.push(structuredClone(automaticResult));
			this.room.add(
				`|tournament|rpgautowin|${p1.name}|${p2.name}|${winner.name}|` +
				`${automaticResult.resolution}|${automaticResult.matchId}`
			);
			this.isBracketInvalidated = true;
			this.isAvailableMatchesInvalidated = true;
		}
	...
	reportRPGBattleResult(participant1Id: string, participant2Id: string, winnerId: string) {
		if (!this.isTournamentStarted) throw new Error('RPG Tournament has not started');
		const id1 = toID(participant1Id);
		const id2 = toID(participant2Id);
		const normalizedWinnerId = toID(winnerId);
		if (!id1 || !id2 || id1 === id2) throw new Error('Invalid RPG Tournament participants');
		const matches = this.generator.getAvailableMatches();
		if (typeof matches === 'string') throw new Error('Unexpected RPG Tournament bracket state: ' + matches);
		const match = matches.find(([p1, p2]) => (
			(p1.rpgParticipant.id === id1 && p2.rpgParticipant.id === id2) ||
			(p1.rpgParticipant.id === id2 && p2.rpgParticipant.id === id1)
		));
		if (!match) throw new Error('RPG Tournament match is not available');
		const [p1, p2] = match;
		if (RPGTournamentSystem.isAutomaticMatch(p1.rpgParticipant, p2.rpgParticipant)) {
			throw new Error('RPG Tournament NPC matches must use automatic resolution');
		}
		const p1Won = normalizedWinnerId === p1.rpgParticipant.id;
		if (!p1Won && normalizedWinnerId !== p2.rpgParticipant.id) {
			throw new Error('RPG Tournament winner is not in the match');
		}
		const result = p1Won ? 'win' : 'loss';
		const score = p1Won ? [1, 0] : [0, 1];
		const winner = p1Won ? p1 : p2;
		...
		p2.games += 1;
		const error = this.generator.setMatchResult([p1, p2], result, score);
		if (error) throw new Error('Unexpected ' + error + ' while reporting RPG Tournament battle');
		this.room.add(
			`|tournament|rpgbattleend|${p1.name}|${p2.name}|${winner.name}|success`
		);
		this.isBracketInvalidated = true;
		this.isAvailableMatchesInvalidated = true;
		this.resolveRPGNPCMatches();
	...
	getAvailableMatches() {
		...
		for (const match of matches) {
			if (match.some(player => player.rpgParticipant.type === 'npc')) {
				// Player/NPC battles are exposed through getRPGPendingMatches and reported by the RPG server.
				continue;
			}
	...
	onTournamentEnd() {
		const update = {
			...
			rpg: {
				participants: this.getRPGParticipants(),
				automaticResults: structuredClone(this.rpgAutomaticResults),
			},
		};
