...
export class RPGIncubation {
	...
	static advance(egg: RPGNurseryEgg, milliseconds: number): {advanced: boolean, completed: boolean} {
		...
		if (egg.status !== 'incubating' || !egg.incubatorId) return {advanced: false, completed: false};
	...
	static hatch(egg: RPGNurseryEgg, incubator: RPGIncubatorState, now: number): RPGHatchResult {
		this.ensureEgg(egg);
		if (egg.status !== 'ready_to_hatch') throw new Error('O ovo ainda n\\u00e3o est\\u00e1 pronto para chocar');
		if (incubator.eggId !== egg.id || egg.incubatorId !== incubator.id) {
			throw new Error('O ovo pronto n\\u00e3o est\\u00e1 nesta incubadora');
		}
